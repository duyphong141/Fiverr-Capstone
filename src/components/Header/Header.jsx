...
import { loaiCongViecAction, searchCongViecAction } from '../../redux/action/action';
import { SEARCH_CONGVIEC } from '../../redux/types/type';
...
export default function Header() {
    ...
    const [maLoaiCV, setMaLoaiCV] = useState(1);
    ...
    const renderLoaiCongViec = () => {
        console.log(typesJob) // console

        return typesJob.map(item => {
            return <li key={item.id} className='dropdown'>
                <NavLink to={`/chitietloai/${maLoaiCV}`}>{item.tenLoaiCongViec}</NavLink>
    ...
    const renderNhomChiTietLoai = (nhomChiTietLoai) => {
        return nhomChiTietLoai.map(item => {
            return <span className='dropdown-item' key={item.id}>
                {item.tenNhom} - {item.maLoaiCongviec}
                {renderdsChiTietLoai(item.dsChiTietLoai)}
                {/* {setMaLoaiCV(item.maLoaiCongviec)} */}
                {/* {console.log(item.maLoaiCongviec)}  */}
            </span>
        })
    }
    ...
    const renderdsChiTietLoai = (dsChiTietLoai) => {
        return dsChiTietLoai.map(item => {
            return <a className='loaiCongViec' href='' key={item.id}>
                {item.tenChiTiet}
            </a>
        })
    }
    ...
    let handleSubmit = (e) => {
        ...
        promise.then((result) => {
            ...
            let action = {
                type: SEARCH_CONGVIEC,
                searchCongViec: result.data.content
            }
            dispatch(action)
        });
        promise.catch(error => {
    ...
    return (
        <div className="main-header1">
            <div className={"thanh-header-top1"}>
                <div className="thanh-header1 d-flex">
                    <a href="/" className="site-logo1">
                        <svg
                            width="89"
                            height="27"
                            viewBox="0 0 89 27"
            ...
            <div className={"thanh-header-bottom1"}>
                <nav className='CategoriesMenu1'>
                    <ul className='categories1 d-flex'>
                        {renderLoaiCongViec()}
