...
export default function Gigs() {
    ...
    useEffect(() => { 
        const layDanhSachDaThue = () =>{
            return() =>{
                ...
                promise.then((result) => { 
                    danhSachCVDaThue= result.data.content;
                 }) 
                promise.catch((error) => { 
                    console.log(error)
                 }) 
    ...
    let renderDanhSachDaThue = () =>{
        return <div className='gigs-list'>
            <div></div>
            <span> <button className=''><i className="fa-solid fa-pencil"></i></button></span>
            <span><button  onClick={(id) => { 
                if(window.confirm("Bạn muốn xoá công việc")){
                    
                }
             }}><i className="fa-solid fa-trash"></i></button></span>
